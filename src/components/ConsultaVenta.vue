    ...
    <v-flex>
      <v-toolbar flat color="white">
        <v-btn @click="crearPdfVentas">
         ...
         <export-excel
          ...
          :data = "ventas"
           ...
           name    = "Ventas totales.xls"
          :fields = "json_fields"
           worksheet = "Ventas de contado y crédito"
          :title ="fecha2"
         type    = "xls"
        ...
        ></v-text-field>
         ...
         <v-flex xs12 sm6 md6>
                    <v-select v-model="idsucursal" :items="sucursales" label="Sucursales">
                    </v-select>
                  </v-flex>
        <v-btn
          v-if="verNuevo == 0"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <div v-if="props.item.estado == 'Aceptado'|| props.item.estado=='CANCELADO' ">
              <span class="blue--text">CANCELADO</span>
            </div>
            <div v-else>
...
export default {
 ...
 selectSucursales() {
      ...
      axios
        .get("api/Usuarios/SucursalSelect", configuracion)
        .then(function (response) {
          sucursalesArray = response.data;
          sucursalesArray.map(function (x) {
            me.sucursales.push({ text: x.nombre, value: x.idsucursal });
          });
        })
    ...
    crearPdfVentas() {
      ...
      // var dateString = d.format
        rows.push({
          ...
          usuario: x.usuario,
          cliente: x.cliente,
          tipo_comprobante: x.tipo_comprobante.toLowerCase(),
      ...
      this.ventas.forEach((element) => {
        if (element.estado == "Aceptado" || element.estado=="CANCELADO")
      ...
      doc.autoTable(columns, rows, {
        ...
        addPageContent: function(data) {
          doc.text("Listado de Facturas: ", 250, 30);
          doc.text(
            "Subtotal de todas las facturas canceladas: " + subtotal.toFixed(2),
            200,
            50
          );
        },
  ...
  heightLeft -= pageHeight;
    ...
    listar() {
      ...
      if (!me.fechaInicio || !me.fechaFin) {
         me.fecha2 ="Reporte de ventas de totales de Crédito y Contado";
        url = "api/Ventas/Listar";
      } else {
        
          me.fecha2 ="Reporte de ventas de Contado y Crédito del "+me.fechaInicio+"  Al  "
       +"   "+me.fechaFin;
