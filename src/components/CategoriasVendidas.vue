  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        ...
        <v-toolbar-title> Categorias Vendidas</v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      ventas: [],
       json_fields: {
            'Número de factura': 'codigoFactura',
            'Nombre de cliente': 'cliente',
            'Tipo de factura': 'tipo_comprobante',
            'Vendedor': 'usuario',
            'Fecha': 'fecha_hora',
            'Subtotal': 'total',
            'Envio': 'impuesto',
  ...
  methods: {
    getTotal(column) {
    return this.ventas.reduce((sum, item) => sum + (parseFloat(item[column]) || 0), 0);
  },
    redirigir() {
...
// Formatear las fechas a solo "día/mes/año"
const fechaInicio = this.fechaInicio;
const fechaFin = this.fechaFin;

// Texto a imprimir
const texto = `Categorías Vendidas del: ${fechaInicio} hasta ${fechaFin}`;
...
doc.text(texto, xPos, 30);
      ...
      doc.save("Categorias Vendidas.pdf");
    },
    ...
    listar() {
      ...
      if (!me.fechaInicio || !me.fechaFin) {
        me.fecha2 ="Reporte de ventas de Contado Totales";
        url = "api/Ventas/ListarContado";
      } else {
        me.fecha2 ="Reporte de ventas de Contado del "+me.fechaInicio+"  Al  "
       +"   "+me.fechaFin;
        url = "api/Ventas/CantidadCategoriaVendidas/" + me.fechaInicio + "/" + me.fechaFin;
      }
      axios
