  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-toolbar-title>Anular Traslados</v-toolbar-title>
        ...
        <template slot="items" slot-scope="props">
          <td class="justify-center layout px-0">
            ...
            <v-icon small class="mr-2" @click="mostrarComprobante(props.item)">
             ...
             <v-icon v-if="props.item.estado!= 'Anulado' " small @click="activarDesactivarMostrar(2, props.item)">
                block
              </v-icon>
          </td>
...
export default {
  ...
  methods: {
    ...
    activarDesactivarMostrar(accion, item) {
      ...
      this.adModal = 1;
      this.adNombre = item.consecutivo;
      this.adId = item.id;
      if (accion == 1) {
        this.adAccion = 1;
      } else if (accion == 2) {
    ...
    desactivar() {
      ...
      let configuracion = { headers: header };
      axios
        .put("api/Articulos/AnularTraslado/" + this.adId, {}, configuracion)
        .then(function (response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
